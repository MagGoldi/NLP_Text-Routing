...
cfg = load_config()
logger = setup_logging(model_name=cfg.models.name, log_dir=cfg.logging.log_dir, level=cfg.logging.level)
...
if cfg.preprocessing.enabled:
    df['result'] = timed_preprocess(df['Текст Сообщения'], cfg.models.name)
...
theme_counts = df['Тематика'].value_counts()
popular_themes = theme_counts[theme_counts > 5].index
df_filtered = df[df['Тематика'].isin(popular_themes)]
...
model_kwargs = dict(cfg.models.params) if cfg.models.name != 'rubert' else {}
if cfg.models.name == 'rubert':
    model_kwargs['num_labels'] = cfg.data.num_labels

model = build_model(cfg.models.name, **model_kwargs)

mlflow_ctx = contextlib.nullcontext()
if cfg.mlflow.enabled:
    init_mlflow(cfg)
    run_name = f"{cfg.models.name}_{datetime.now():%Y%m%d_%H%M%S}"
...
with mlflow_ctx:
    ...
    logger.info("Обучение начато: model=%s fine_tune_mode=%s", cfg.models.name, cfg.models.fine_tune_mode)

    if cfg.models.name == 'rubert':
        train_kwargs = dict(cfg.models.params)
        train_kwargs['callbacks'] = [FileLoggingCallback(logger)]
        if cfg.mlflow.enabled and cfg.models.fine_tune_mode == 'full':
        ...
        model_fit = model.train(train_loader, val_loader, fine_tune_mode=cfg.models.fine_tune_mode, **train_kwargs)
    else:
    ...
    viz = Visualizer(output_dir="data/visualizations", model_name=cfg.models.name, dpi=300, top_k=10)
    ...
    feature_names = vec.get_feature_names_out() if cfg.models.name != 'rubert' else None
    ...
    if fi_values is not None:
        ...
        logger.warning("Модель '%s' не поддерживает извлечение важности признаков — график пропущен.", cfg.models.name)
    ...
    log_run(model_name=cfg.models.name, f1=summary['f1_macro'][0], duration_sec=run_duration, log_path=run_log_path)
    ...
    if cfg.mlflow.enabled:
        ...
        log_model_artifact(cfg.models.name, model_fit)
